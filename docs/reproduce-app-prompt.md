...
Build a **single-file web app** (`index.html`) called **"My Rafa Voucher Tracker"** — a mobile-first voucher attendance tracker used by a shift worker. The user tracks their **shift pattern** (e.g. 2-2-2: 2 morning shifts, 2 night shifts, 2 days off), marks missed working days, sees attendance stats per 4-week "voucher period", records the day and amount they get paid per voucher period, and can switch to **any** repeating shift pattern at any time.
...
### 3.3 Paid summary card (right after the stats row)
Hidden by default (`hidden` class). Shown by `updatePaidSummary` when the viewed period has a payment.
```html
<div id="paid-summary" class="mx-3 sm:mx-5 mt-3 hidden rounded-2xl overflow-hidden shadow-lg">
  <div class="bg-gradient-to-br from-emerald-500 via-green-500 to-teal-500 text-white px-5 py-4">
    <div class="flex items-center justify-between gap-3">
      ...
      <div class="text-center shrink-0 bg-white/15 rounded-2xl px-4 py-2">
        <p class="text-2xl font-bold" id="paid-summary-days">0</p>
...
.paid-popup {
    ...
    color: #22c55e; -webkit-text-stroke: 2px white; paint-order: stroke fill;
    text-shadow: 0 0 18px rgba(34,197,94,0.65);
    transform-origin: center center;
    animation: paidBounce 1.2s cubic-bezier(.34,1.56,.64,1) forwards;
}
@keyframes paidBounce {
    0%   { opacity: 0; transform: scale(0.3) rotate(-10deg); }
    30%  { opacity: 1; transform: scale(1.35) rotate(4deg); }
    50%  { transform: scale(0.95) rotate(0deg); }
    70%  { transform: scale(1.12); }
    85%  { transform: scale(1); }
    100% { opacity: 1; transform: scale(1); }
}
#paid-summary { animation: paidSummaryIn 0.5s ease-out; }
@keyframes paidSummaryIn {
    from { opacity: 0; transform: translateY(-8px) scale(0.97); }
    to   { opacity: 1; transform: translateY(0) scale(1); }
}
@media (prefers-reduced-motion: reduce) {
    .paid-popup, #paid-summary { animation: none !important; }
}
...
```
> **Fidelity note:** the doc's CSS block is split across multiple `<style>` sections in the real file, and there are **two** `@media (prefers-reduced-motion: reduce)` blocks — the cosmetics block (`.help-attention`, `.help-tooltip`) and a second block (`.paid-popup`, `#paid-summary`). Multiple reduced-motion blocks are valid and intentional.
...
### 5.3 Day-action modal (`#day-action-modal`, `#day-action-backdrop`)
Both new modals are fixed overlays: `hidden fixed inset-0 z-50 flex items-center justify-center px-4`, `role="dialog" aria-modal="true"`, with an absolute `<div class="absolute inset-0 bg-black/50 backdrop-blur-sm">` backdrop and a `relative w-full max-w-sm bg-white rounded-3xl shadow-2xl z-10 overflow-hidden` card.
- Header: gradient `from-indigo-500 to-purple-600 px-6 py-4 text-white`; `#day-action-title` (`text-lg font-bold`, shows "Jul 20 – Aug 19, 2026 Voucher"); `#day-action-date` (`text-xs text-indigo-100`, shows weekday + date e.g. "Sat, Aug 24").
- Body: `px-6 py-5 text-sm text-gray-700`; label "Choose an action for this day" + `<select id="day-action-select" class="w-full border border-gray-300 rounded-lg px-3 py-2.5 bg-white focus:outline-none focus:ring-2 focus:ring-indigo-400">` (options populated by JS — see §8.7a).
- Footer: `px-6 py-4 bg-gray-50 border-t border-gray-100 flex items-center justify-between`; `#day-action-cancel` (**Cancel**, gray) and `#day-action-continue` (`bg-indigo-600 hover:bg-indigo-700 text-white font-bold px-8 py-2.5 rounded-full` → **Continue**).
...
### 6.1b Helper utilities (exact)
```js
function keyOf(d){const y=d.getFullYear();const m=String(d.getMonth()+1).padStart(2,'0');const dd=String(d.getDate()).padStart(2,'0');return `${y}-${m}-${dd}`;}
...
function getPaidForPeriod(startDate, endDate){
    if(!paidDays.length) return null;
    const sk=keyOf(startDate), ek=keyOf(endDate);
    return paidDays.find(p=>p.date>=sk && p.date<=ek) || null;
}
function formatNaira(amount){ return '\u20A6' + amount.toLocaleString('en-NG',{minimumFractionDigits:2, maximumFractionDigits:2}); }
function savePaidDays(){ localStorage.setItem('paidDays', JSON.stringify(paidDays)); }
```
...
```
> Note: popups must be shown **before** `renderCalendar()` (the grid rebuild detaches the button, so `getBoundingClientRect()` is read first).
...
All day clicks outside cycle-setting mode open a modal dialog. Opening it (exact, matching the implementation):
```js
function openDayDialog(btn, dt, date) {
    actionDay = date;                       // string 'YYYY-MM-DD'
    actionBtn = btn;
    const { startDate, endDate } = getVoucherPeriod(periodOffset);
    actionPeriod = { start: startDate, end: endDate };
    document.getElementById('day-action-date').textContent =
        `${dt.toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })}`;
    document.getElementById('day-action-title').textContent = `${rangeLabel(startDate, endDate)} Voucher`;
    const select = document.getElementById('day-action-select');
    select.innerHTML = '';
    ...
    if (paidDays.some(p => p.date === date)) {
        const paid = paidDays.find(p => p.date === date);
        select.appendChild(opt('', `Already paid \u00B7 ${formatNaira(paid.amount)}`, true));
    } else {
        const shift = getShiftInfo(dt);
        if (missedDays.includes(date)) select.appendChild(opt('unmiss', 'Unmiss'));
        else if (shift && shift.work) select.appendChild(opt('miss', 'Missed'));
        select.appendChild(opt('paid', 'Paid'));
        select.appendChild(opt('copy', 'Copy Date'));
    }
...
function applyDayAction() {
    if (!actionDay) return;
    const val = document.getElementById('day-action-select').value;
    const btn = actionBtn;
    if (val === 'miss') {
        missedDays.push(actionDay);
        localStorage.setItem('missedDays', JSON.stringify(missedDays));
        closeDayDialog(); showMissedPopup(btn); renderCalendar();
    } else if (val === 'unmiss') {
        missedDays = missedDays.filter(d => d !== actionDay);
        localStorage.setItem('missedDays', JSON.stringify(missedDays));
        closeDayDialog(); showUnmissedPopup(btn); renderCalendar();
    } else if (val === 'copy') {
        closeDayDialog(); showCopyToast(actionDay);   // navigator.clipboard.writeText(actionDay)
    } else if (val === 'paid') {
        closeDayDialog(); openPaidAmountDialog();
    }
}
```
Dialog option rules (exact): if the day is already paid → only a single disabled option `Already paid · ₦X` (no other actions offered). Otherwise: `Unmiss` if already missed, else `Missed` if it is a work day; then always `Paid` and `Copy Date`. OFF days offer `Paid` + `Copy Date` only.

### 8.7b Paid amount dialog
```js
...
function closePaidAmountModal() {
    document.getElementById('paid-amount-modal').classList.add('hidden');
    document.body.style.overflow = '';
}
function submitPaidAmount() {
    ...
    if (!actionDay) return;
    paidDays = paidDays.filter(p => p.date !== actionDay)
        .concat({ date: actionDay, amount: Math.round(amt * 100) / 100 });
    savePaidDays();
    const btn = actionBtn;
...
- `day-action-continue` → `applyDayAction()`
...
> - **Tap** any day to open an **action dialog** (its title is the current voucher period).
> - Choose **"Missed"** from the drop-down to mark that day as missed &mdash; it turns **bold red**.
> - If the day is already missed, the drop-down shows **"Unmiss"** &mdash; tap it to restore the day.
> - OFF days cannot be marked missed &mdash; days off are already rest days.
...
> - **Copy a date** &mdash; open a day&rsquo;s action dialog and choose **"Copy Date"** to copy its YYYY-MM-DD to your clipboard.
> - **Missed popup** &mdash; when you mark a day missed, a "MISSED" animation flashes on that day.
> - **Un-missed popup** &mdash; tap the missed day again to clear it; a "UNMISSED ✓" animation appears.
> - **Anchor flash** &mdash; when you set or change your cycle start, an "ANCHOR ✓" animation confirms it.
> - **PAID popup** &mdash; when you record a payment, a green "PAID ✓" animation flashes on that day.
...
12. **Export CSV:** header `Date,Day,Shift,Status`; PRESENT/MISSED/OFF values; `--` only if no anchor (blocked by the guard alert).
13. **Import:** restore a backup → identical state (including paidDays); malformed file → `alert`; theme restored.
14. **Reset:** confirm dialog; everything clears (including paidDays); button back to "Set Cycle Start".
15. **Dark mode:** toggle flips `data-theme`, knob slides 28px, icon ☀️/🌙; all overrides in §4 apply (card, stats row, buttons, help modal, footer).
16. **Persistence:** reload the page → anchor, missed marks, patterns, paid records, theme all retained.
17. **Accessibility:** every interactive element has a meaningful `aria-label` or `title`; modals have `role="dialog"`/`aria-modal`; grid has `role="grid"`.
